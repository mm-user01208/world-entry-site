...
export const metadata: Metadata = { title: 'Australia' };
    ...
    <>
      <section className="bg-gradient-to-br from-blue-700 to-blue-900 text-white py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h1 className="font-serif-country text-3xl md:text-4xl font-bold mb-4">Australia</h1>
          <p className="text-blue-200">オーストラリアのビザ・電子渡航認証（ETA）情報</p>
        </div>
      </section>
      ...
      <article className="max-w-4xl mx-auto px-4 py-12 space-y-10">
        ...
        <Section title="オーストラリアのビザとは">
          ...
          <p className="mt-4">オーストラリアのビザは、観光や短期商用を目的とした短期滞在向けのものから、就労、留学、家族との同居や長期滞在を目的としたものまで、目的別に多くの種類が存在します。短期間の滞在であっても、内容によっては就労や報酬を伴う活動が制限される場合があり、ビザの種類によって許可される範囲は大きく異なります。</p>
          <p className="mt-4">また、近年は手続きの簡素化が進み、オンラインや公式アプリを通じて申請・発給されるビザも増えています。これらは一般的に電子的に管理され、紙のビザが発行されないケースが多く、パスポート情報と紐づけて管理されます。一方で、滞在期間が長期に及ぶ場合や、特定の条件が求められるビザでは、追加書類の提出や詳細な審査が行われます。</p>
          <p className="mt-4">このように、オーストラリアのビザは「滞在中に何ができるか」を定める重要な要素であり、制度の違いを理解したうえで、自身の渡航目的に合ったビザを選択することが大切です。</p>
        ...
        <Section title="短期滞在向けビザ（観光・商用）">
          <h3 className="font-bold text-gray-800 mt-4 mb-2">電子的に申請できる短期ビザ</h3>
          ...
          <p className="mt-4">ETAは、主に日本を含む対象国・地域の渡航者が利用でき、観光や短期の非就労商用活動を目的とした滞在が認められています。一方、eVisitorは主に欧州の一部の国籍を対象とした短期滞在用ビザで、ETAと同様に観光や短期商用を目的とした利用が可能です。</p>
          
          <h3 className="font-bold text-gray-800 mt-6 mb-2">観光ビザ（Visitor visa / Subclass 600）</h3>
          <p>観光ビザ（Visitor visa / Subclass 600）は、観光、知人・家族訪問、短期の非就労活動などを目的として利用される短期滞在向けビザです。電子的に申請可能な短期ビザの対象外となる国籍の方や、より長い滞在期間を希望する場合に選択されることがあります。</p>
        </Section>

        <Section title="ETA（Electronic Travel Authority / Subclass 601）">
          <p>オーストラリアの ETAは、観光や短期商用を目的とする渡航者向けに設けられた短期滞在用のビザです。一般的に「電子渡航認証」と呼ばれることもありますが、制度上はビザの一種として位置づけられており、入国や滞在の条件が明確に定められています。ETAは、オーストラリア政府が提供する公式アプリを通じて申請・発給され、承認後はパスポート情報と電子的に紐づけられます。紙のビザは発行されず、渡航時に別途書類を携帯する必要はありません。主に観光、知人・家族訪問、短期の商用活動が認められますが、就労や長期滞在、留学を目的とした利用は認められていません。</p>
        </Section>

        <Section title="学生ビザ（Student visa）">
          <p>オーストラリアの学生ビザ（Student visa）は、大学や専門学校、語学学校など、政府に認可された教育機関で就学することを目的としたビザです。主な目的は学業であり、就労や長期滞在を主目的とするビザとは制度上明確に区別されています。</p>
          <p className="mt-4">学生ビザでは、在学期間中の滞在が認められるほか、一定の条件下で就労が許可される場合があります。ただし、就労時間や内容には制限が設けられており、学業に支障が出ない範囲でのみ認められる点に注意が必要です。</p>
        </Section>

        <Section title="就労・家族支援タイプの長期ビザ">
          ...
          <p className="mt-4">たとえば、雇用主の支援を受けて就労を行うことを前提としたビザの中には、一定の条件を満たすことで永住権につながるものも存在します。また、家族との同居や扶養関係を理由として申請されるビザであっても、滞在期間が長期に及び、結果として永住を目的とするケースもあります。</p>
        </Section>

        <Section title="ビザ利用時の注意点">
          ...
          <p className="mt-4">また、取得したビザには、認められる滞在期間や活動内容が明確に定められており、その条件を超えた行為は認められていません。たとえば、就労が許可されていないビザで働くことや、滞在期間を超えて滞在することは、ビザ条件違反となる可能性があります。トラブルを避けるためにも、渡航前にビザの条件を十分に理解し、遵守することが重要です。</p>
        </Section>
      </article>
